package org.osmf.media.videoClasses
{
	...
	internal class VideoSurfaceManager
	{
		...
		{
			trace("-->","REG VIDEO SURFACE")
			videoSurface.addEventListener(WrapperEvent.STAGE, onAddedToStage);
//			videoSurface.addEventListener(Event.REMOVED_FROM_STAGE, onRemovedFromStage);
		 ...
		 */
		internal function registerStage(stage:Object):void
		{
...
//			stageVideoIsAvailable = _stage.hasOwnProperty("stageVideos") && _stage.stageVideos.length > 0;
		 ...
		 */
		private function onStageVideoAvailability():void
		{
			...
			trace("-->","CURRENT STATE", stageVideoIsAvailable, "BECOME", _stage.hasOwnProperty("stageVideos") && _stage.stageVideos.length > 0)
			if (stageVideoIsAvailable != _stage.hasOwnProperty("stageVideos") && _stage.stageVideos.length > 0)
			{
				CONFIG::LOGGING
				{
					logger.info("stageVideoAvailability changed to " + _stage.hasOwnProperty("stageVideos") && _stage.stageVideos.length > 0);
				}
				stageVideoIsAvailable = _stage.hasOwnProperty("stageVideos") && _stage.stageVideos.length > 0;
